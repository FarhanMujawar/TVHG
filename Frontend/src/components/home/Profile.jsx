...
export default function AnchorTemporaryDrawer() {
  ...
  const list = (anchor) => (
    <Box
      sx={{ width: anchor === 'top' || anchor === 'bottom' ? 'auto' : anchor === 'right' ? '50%' : 250 }} // Set the right drawer to 50% width
      role="presentation"
      onClick={toggleDrawer(anchor, false)}
      onKeyDown={toggleDrawer(anchor, false)}
    >
      {/* Conditionally render Lorem component only on the right side */}
      {anchor === 'right' ? (
        <Lorem />
      ) : (
        <>
